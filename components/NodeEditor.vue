<template>
    ...
    <template v-if="typeof value === 'object'">
        <template v-for="i in Object.keys(value)">
            <div class="relative 
            gap-1
            current_node bg-base-300 flex items-center  px-2 py-1   text-sm rounded space-x-1 ">
                <svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
                </svg>
                <p class="z-10">{{ i }}</p>
                <div class="flex-1" v-if="!isObject(value[i])">
                    <NodeEditor v-model:kv="value[i]" :locked="locked" ></NodeEditor>
                </div>
            </div>
            <div class="child_nodes">
                <template v-if="typeof value[i] === 'object'">
                    <div class=" space-y-2 vertical_line relative border-base-300  border-l-2 ml-2 pl-2  my-1 ">
                        <NodeEditor v-model:kv="value[i]" :locked="locked"></NodeEditor>
                    </div>
                </template>
            </div>
